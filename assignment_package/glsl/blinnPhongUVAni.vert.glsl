...
in vec4 vs_Col; //bogus data
in vec2 vs_UV; //For UV mapping
in float vs_CosPow; //For how shiny the object would be
in float vs_Animatable;

out vec4 fs_LightVec; // The direction in which our virtual light lies, relative to each vertex. This is implicitly passed to the fragment shader.
out vec4 fs_CameraPos;
out vec4 fs_Pos;            // position passed to fragment for coloring purposes
...
void main()
{
    ...
    fs_Animatable = vs_Animatable; // Pass the flag whether to animate texture to the fragment shader for interpolation
    fs_CosPow = vs_CosPow; // Pass the vertex UVs to the fragment shader for interpolation
    fs_CameraPos.xyz = u_CameraPos;
    fs_PosLightSpace = u_LightSpaceMatrix * fs_Pos;//shadow mapping
}
